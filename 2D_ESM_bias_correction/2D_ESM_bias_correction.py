...
for i in range(0,len(modelLs)):
    for scen in scenarios:
        ...
        if scen == 'historical':
            ...
            time_s = np.arange(1950, 2015, 10)
            time_f = np.append(np.arange(1959, 2010, 10), 2014) # last period is 5y
        else:
            # time_s = np.arange(2015, 2099, 10)
            # time_f = np.arange(2024, 2100, 10)
            time_s = np.append(2015, np.arange(2020, 2099, 10) ) # 5y, then 10y
        ...
        for t in range(0,len(time_s)):
            ...
            print('   ', dsOut.time.values.min().astype('datetime64[h]'), ' to ', dsOut.time.values.max().astype('datetime64[h]'), ' written to' )
            dsOut.to_netcdf(out_dir+'/'+modelLs[i]+'/'+scen+'/' + modelLs[i] + '_bias_corr_final_'+start_year+'-'+end_year+'.nc')
            print('   ', out_dir+'/'+modelLs[i]+'/'+scen+'/' + modelLs[i] + '_bias_corr_final_'+start_year+'-'+end_year+'.nc')
...
print(" \n    - - - - Finished 2D ESM Bias Correction !  - - - - ") 
